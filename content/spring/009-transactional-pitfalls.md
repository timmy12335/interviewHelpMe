...
失效的情境很多，但根源只有四類，按根源講比較有系統。

第一類是代理沒生效。最常見的是自我呼叫，同一個類別裡 A 直接呼叫 B，走的是 this，繞過代理，註解等於沒寫。解法是把 B 抽到另一個 Bean，或用自注入。方法不是 public 也一樣，Spring 的交易只認 public。物件如果是自己 new 的、不在容器裡，那更是沒有代理。

第二類是回滾條件不滿足。方法裡把異常 catch 掉又沒重新拋出，攔截器以為一切正常，就提交而不是回滾。另一種是受檢異常，預設只回滾 RuntimeException，受檢異常要自己設 rollbackFor。

第三類是交易上下文丟失。交易綁在 ThreadLocal 上，把資料庫操作丟進 @Async 或執行緒池，新執行緒拿不到上下文，那些操作就不在原交易裡。

第四類是底層不支援，像 MyISAM 引擎本來就沒有交易，得換 InnoDB。
