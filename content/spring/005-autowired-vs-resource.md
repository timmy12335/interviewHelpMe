...
**核心答案**：`@Autowired` 可以標在——欄位、建構子、setter 方法、以及普通方法（參數會被注入）上。欄位注入（直接標在欄位上）雖然最簡潔，但不被推薦，因為——它讓依賴變成「隱藏的」（從類別的公開 API/建構子看不出這個類別依賴什麼）、無法把欄位宣告為 `final`（不能保證依賴不可變）、且脫離 Spring 容器就無法注入（測試時要靠反射或啟動容器，不能簡單地 new 出來傳 mock）。相比之下建構子注入沒有這些問題，是現在推薦的方式。
