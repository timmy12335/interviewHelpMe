...
InterviewHelpMe 第六個內容類別，共 24 題，涵蓋 MySQL/InnoDB 索引原理（B+ 樹、聚簇索引、聯合索引、索引失效、覆蓋索引、EXPLAIN）、交易與併發控制（ACID、隔離級別、MVCC、鎖類型、間隙鎖、樂觀鎖與悲觀鎖）、崩潰恢復與複製（redo log、binlog 與兩階段提交、讀寫分離與主從延遲）、效能優化（深分頁、慢查詢排查）、以及分散式資料庫工程（分庫分表、分散式 ID、正規化與反正規化、儲存引擎比較、連線池、SQL 注入防範）。
