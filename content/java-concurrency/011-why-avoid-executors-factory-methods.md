...
**正確做法**：直接使用 `new ThreadPoolExecutor(...)`，明確指定一個**有界佇列**與合理的 `maximumPoolSize`，並搭配自訂拒絕策略。
...
**核心答案**：可以透過監控佇列長度並設定告警閾值提早發現積壓、限制單一任務攜帶的資料量避免大物件佔用過多記憶體、以及在應用層額外加一層限流（如 `Semaphore`）控制提交速度，從源頭減少湧入佇列的任務量。
