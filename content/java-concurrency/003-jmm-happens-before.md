...
JMM 存在的理由是，CPU 有多級快取，編譯器和處理器又會重排指令。單執行緒下重排不影響結果，多執行緒下卻讓可見性變得不確定。JMM 就是規範這件事的模型。

happens-before 是 JMM 描述可見性的偏序關係。兩個操作滿足它，前者的記憶體效果就保證對後者可見，即使跑在不同執行緒。synchronized、volatile 和 juc 工具類底層都是靠它保證可見性。

規則挑幾條講。同一執行緒內，前面的操作 happens-before 後面的。解鎖 happens-before 後續對同一把鎖的加鎖，所以 B 拿到鎖時看得到 A 在鎖內寫的東西。volatile 的寫對之後的讀成立，start 之前的寫入對該執行緒的操作成立，這些關係有傳遞性。

最後澄清一個誤區。happens-before 講的不是時間先後，是可見性。就算 A 先跑，沒建立這層關係，B 還是可能看不到。
