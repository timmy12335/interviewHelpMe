...
我的理解是，為什麼這樣設計能支撐這麼多不同工具。

再來，子類別只需要決定 state 怎麼解讀，排隊喚醒都是共用邏輯。

要區分清楚的是，這是 Doug Lea 用模板方法模式（Template Method Pattern）解決『重複造輪子』問題。

## 常見追問

### ReentrantReadWriteLock 如何用一個 state 同時表示讀鎖與寫鎖的狀態？

**核心答案**：把 32 位元的 `state` 拆成高 16 位與低 16 位，高 16 位表示讀鎖被持有的次數（共享鎖（Shared Lock）計數），低 16 位表示寫鎖的重入次數（獨佔鎖計數）。
