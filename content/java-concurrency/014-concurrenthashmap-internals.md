...
JDK 7 的做法是分段鎖。整張 Map 由多個 Segment 組成，每個 Segment 就是一個獨立的小型 HashMap，而 Segment 繼承自 ReentrantLock。寫入只鎖住資料所在那一段，預設併發度 16。問題是 Segment 數量初始化後固定，並行度上限被鎖死。

JDK 8 把 Segment 整個拿掉，直接用 Node 陣列。寫入先用 CAS 試著把節點放進空桶，這一步完全無鎖。桶裡已經有節點，才對頭節點加 synchronized，鎖只影響這一個桶。

鏈結串列長度超過 8、陣列容量也夠大時，會轉成紅黑樹，查詢從 O(n) 降到 O(log n)。讀取則因為 table 和 Node 的關鍵欄位都是 volatile，多數情況不用加鎖。

這條演進線背後就是鎖粒度細化。從段鎖縮到桶鎖，鎖保護的範圍越小，能同時進來的執行緒越多。資料庫的表鎖跟行鎖是同個道理。
