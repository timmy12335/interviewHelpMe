...
Optional 是 JDK 8 的容器，表示這裡可能有值、也可能是空的。

它真正的價值不在少寫幾個 null 判斷，而是把「可能為 null」從隱藏的執行期陷阱，變成方法簽章上明確可見的契約。以前方法回傳 User，呼叫方根本不知道會不會回 null；回傳 Optional User 就是在型別層面說清楚，這裡可能沒有值，你必須處理。

正確用法主要是當回傳值，配 map、filter、orElse 鏈式處理，讓「有值怎樣、沒值怎樣」一行表達完。

更值得講的是幾個反模式。當欄位會多一層包裝，而且它不可序列化，很多框架會出問題。當方法參數的話，呼叫方還是得包一層，比多寫一個多載更麻煩。拿到之後直接 get 不檢查，等於把 NullPointerException 換成 NoSuchElementException，只是換了個例外名字。

所以我的原則是：Optional 用在回傳值的邊界上表達意圖，不要讓它滲透到欄位、參數和資料結構裡。
