...
Optional 是 JDK 8 加進來的一個容器，表示這裡可能有值、也可能是空的。

它真正的價值不在於少寫幾個 null 判斷，而在於把「可能為 null」這件事從一個隱藏的執行期陷阱，變成方法簽章上明確可見的契約。以前一個方法回傳 User，呼叫方根本不知道它會不會回 null，只能靠文件或踩過坑才知道；回傳 Optional User 就是在型別層面告訴呼叫方，這裡可能沒有值，你必須處理空的情況。

正確用法主要是當作方法的回傳值，然後用 map、filter、orElse 這些方法鏈式處理，讓「有值就怎樣、沒值就怎樣」在一行裡表達完，而不是拆成 if else。

不過我覺得更值得講的是幾個反模式。第一是拿 Optional 當欄位，這會增加一層包裝、而且 Optional 本身不可序列化，很多框架會出問題。第二是拿它當方法參數，呼叫方還是得包一層，比直接多寫一個多載更麻煩。第三是拿到 Optional 之後直接呼叫 get 不檢查，那等於把 NullPointerException 換成 NoSuchElementException，完全沒有解決問題，只是換了個例外名字。

所以我的原則是：Optional 用在回傳值的邊界上，表達意圖；不要讓它滲透到欄位、參數和資料結構裡面去。
