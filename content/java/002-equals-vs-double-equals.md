...
這題要拆成兩種情況談。基本型別像 int、double，== 就是單純比值，1 == 1 為 true，這裡沒有物件也沒有引用的概念。

換成引用型別，== 比的是兩個變數有沒有指向同一塊堆記憶體。所以 new String("a") == new String("a") 是 false，new 產生了兩個物件，位址不同，即使內容都是 a。

equals() 是 Object 定義的方法，預設實作就是 return this == other，一樣在比引用。差別在於 String、Integer 這些類別把它覆寫掉，改成比內容。同一個例子換成 equals()，答案就變 true。

實務上比字串內容一定用 equals，或用 Objects.equals 順便避開 NPE。拿 == 比字串有時碰巧成立，因為字面量共用字串池，有時又不成立，行為不一致，很難查。
