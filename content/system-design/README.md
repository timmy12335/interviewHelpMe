...
InterviewHelpMe 第八個內容類別，共 50 題，涵蓋系統設計方法論與基礎（面試流程、擴展性、容量估算）、基礎設施建構模組（負載平衡、正反向代理、CDN、快取策略與一致性問題）、經典系統設計案例（短網址、通知系統、即時聊天、社群動態消息、搜尋自動完成、分散式檔案儲存、影音串流、網路爬蟲、即時排行榜、叫車派單）、維運與資料處理主題（部署策略、異地容災、批次/流式處理、監控告警、資料分區），**分散式儲存深入主題**（限流器系統設計、鍵值儲存、quorum 共識、向量時鐘、gossip 故障偵測、hinted handoff、Merkle Tree 反熵），以及**第二批案例與判斷題**（支付、秒殺、票務、排程、協同編輯、廣告統計、地理搜尋、時序儲存、計費、推薦服務、內容審核、遊戲同步、郵件，加上熱點處理、一致性邊界、成本、可用性層級與取捨方法論）。
...
- **001-024** 是基礎建構模組與經典案例，建立系統設計的共同詞彙。
- **025-032** 對照 Alex Xu 的章節結構補上分散式儲存的深入主題。
- **033-050** 補上第二批設計案例與**判斷型題目**。案例部分挑的是前面沒有涵蓋的形態——**強一致的交易類**（支付、秒殺、票務）、**資料密集型**（廣告統計、時序儲存、計費）、**即時互動類**（協同編輯、遊戲同步）、以及**帶模型的服務**（推薦、內容審核）。判斷部分（046-050）不設計具體系統，而是問**熱點怎麼系統性處理、一致性邊界該畫在哪、成本從哪裡來、可用性每多一個 9 要付出什麼、以及面試中怎麼展現取捨**。
...
| 033 | 設計支付系統（Payment System） | hard |
| 034 | 設計秒殺與限量搶購系統（Flash Sale） | hard |
| 035 | 設計分散式任務排程系統（Job Scheduler） | medium |
| 036 | 設計協同編輯系統（OT 與 CRDT） | hard |
| 037 | 設計票務與座位預訂系統 | medium |
| 038 | 設計廣告點擊統計系統 | hard |
| 039 | 設計附近的人與地理搜尋服務 | medium |
| 040 | 設計指標監控與時序儲存系統 | medium |
| 041 | 設計用量計量與計費系統 | medium |
| 042 | 設計推薦系統的服務架構 | medium |
| 043 | 設計內容審核系統 | medium |
| 044 | 設計多人線上遊戲的狀態同步 | hard |
| 045 | 設計郵件系統（Email System） | medium |
| 046 | 熱點與資料傾斜的系統性處理 | hard |
| 047 | 一致性的邊界：強一致與最終一致該畫在哪裡 | hard |
| 048 | 系統設計中的成本估算與取捨 | medium |
| 049 | 高可用的層級：從 99.9% 到 99.99% 要付出什麼 | medium |
| 050 | 系統設計的整體判斷：面試中怎麼做取捨 | hard |

難度分布：easy 4、medium 28、hard 18（中等題最多；025–032 與 033-050 這兩批偏深入，hard 比例較高）。

> 原本規劃在 025–032 這批裡的 LSM Tree 與 SSTable 一題已移至 [`database/025`](../database/025-lsm-tree-sstable.md)——它談的是單節點儲存引擎，與資料庫類別的 B+ 樹題目關係更近。033 之後的編號由第三批內容接續使用。
