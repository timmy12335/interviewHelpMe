...
這兩個常被混著講，其實一個是結果，一個是原因。

記憶體溢位是結果。JVM 分配不到足夠的空間給新物件，就拋 OutOfMemoryError。可能是洩漏，也可能只是堆設太小，或一口氣分配了超大物件。

記憶體洩漏則是程式缺陷。物件在業務上不需要了，可是從 GC Roots 到它還連著一條引用鏈，可達性分析判定它活著，GC 就不回收。單一次洩漏不痛不癢，累積久了才吃光記憶體。洩漏會導致 OOM，但 OOM 不一定來自洩漏。

常見來源舉幾個。static 集合最經典，static 變數本身就是 GC Root，物件丟進去忘了移除就永遠可達。監聽器和回呼註冊完沒註銷，被事件源抓著。執行緒池裡用 ThreadLocal 沒 remove，value 殘留在執行緒上。連線、檔案流忘記關。非靜態內部類別隱式持有外部實例，活得比外部久就把外部拖住。自己寫的快取沒有上限或過期策略，物件只進不出。
