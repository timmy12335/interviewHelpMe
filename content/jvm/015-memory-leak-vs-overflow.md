...
記憶體溢位（OOM）是「結果」，記憶體不夠用了，申請不到需要的空間，拋 OutOfMemoryError。記憶體洩漏是「原因之一」，本該被回收的物件因為仍被某個引用持有而無法回收，這些洩漏的物件不斷累積，最終耗盡記憶體導致 OOM。

兩者的聯繫是，記憶體洩漏累積到一定程度會導致記憶體溢位，但 OOM 不一定是洩漏造成的（也可能是單純的容量不足、或瞬時分配過大）。常見洩漏原因包括：集合類持有物件不釋放、監聽器/回呼未移除、ThreadLocal 未 remove、資源未關閉、內部類別隱式持有外部類別引用等。
