...
Liveness 失敗會重啟容器，它問的是這個行程還有沒有救。Readiness 失敗會把 Pod 從 Endpoints 移除、停止導流量，但不重啟，它問的是現在能不能接請求。Startup 失敗也會重啟，但在它成功之前，另外兩個探針都被暫停。

設計時有一條原則我覺得最關鍵：Liveness 檢查自己，Readiness 才檢查依賴。

重啟能修好的問題才屬於 Liveness，比如死結、卡住的行程。資料庫連不上、下游逾時，重啟這個 Pod 完全沒用，那是 Readiness 的範圍。

違反這條原則的後果很具體。如果 Liveness 也去檢查資料庫，下游一抖，整批 Pod 會被同時重啟，一個小故障就被放大成全面雪崩。

Startup 探針解決的是慢啟動被誤殺。一個 JVM 應用可能要九十秒才起得來，沒有它你只能把 Liveness 的初始延遲設得很長，代價是應用啟動後真的死掉，也要等很久才被發現。有了它就可以啟動期寬鬆、啟動完立刻切成緊湊設定。

實務上還有一個常見坑：探針的逾時預設只有一秒，高負載時很容易誤判，這是 Pod 莫名其妙一直重啟的常見原因。
