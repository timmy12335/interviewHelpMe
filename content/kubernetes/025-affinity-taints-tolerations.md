...
**面試回答方式**：主張**按資源型態與隔離需求劃分而非按團隊**，並列出合理的池類型。核心加分點是說明**按團隊劃分通常錯誤**——**每個池都要各自預留冗餘，整體利用率大幅下降，而 K8s 的價值恰恰在共用資源池**；**成本歸屬應該用標籤與成本工具而非實體隔離**，**真正需要隔離的只有合規、噪音鄰居、硬體不同三種**。收在 spot 節點：**便宜 60% 到 90% 但隨時被回收**，**前提是 PDB 正確與應用能優雅處理終止訊號**。
